...
"#,
                                ...
                                (arg_name, rust_type.to_upper_camel_case().into())
                            }
...
"#,
                ...
                // Ignore all other kinds of world export
                _ => {}
            }
        }
    }
    ...
    fn visit_item_mut(&mut self, node: &mut syn::Item) {
        match node {
                ...
                if t.ident.to_string() == HOST_IMPORTS_TRAIT_NAME {
                    for ti in t.items.iter() {
                        match ti {
                            ...
                            // Ignore non-method entries in traits
                            _ => {}
                        }
                    }
                }
            ...
            Item::Struct(s) => {
                ...
                {
                    debug_print(format!(
                        "detected serialize/deserialize to derive on struct {}",
                        s.ident.to_string(),
                    ));
                    ...
                    debug_print(format!(
                        "successfully added serialize/deserialize derive on {}",
                        s.ident.to_string(),
                    ));
